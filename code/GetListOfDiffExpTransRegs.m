function GetListOfDiffExpTransRegs(GO, strict)
...
if strict
  fid=fopen('DiffExpGenesTFStatusStrict.tsv','w+');
else
  fid=fopen('DiffExpGenesTFStatus.tsv','w+');
end  
tfInds = [];
psIsTR = zeros(1,N);
psIsTF = zeros(1,N);
...
if strict
  save 'DiffExpGenesTFStatusStrict' 'psIsTR' 'psIsTF' 'masterInds';
else
  save 'DiffExpGenesTFStatus' 'psIsTR' 'psIsTF' 'masterInds'; 
end 
